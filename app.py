...
if "stored_data" not in st.session_state:
    st.session_state["stored_data"] = {} #! Store encrypted data
...
if "cipher" not in st.session_state:
    ...
    st.session_state["cipher"] = Fernet(key) #! Generating a new key for encryption
    ...
    if st.button("Retrieve"):
        ...
        if username in stored_data:
            ...
            if hash_password(passkey) == user_data["passkey"]:
                ...
                user_data["failed_attempts"] = 0  # reseting attempts on success
            else:
...
def logout():
    st.session_state["auth"] = False
    st.session_state.clear()  # Clear session state including user data
...
def main():
    ...
    if not st.session_state["auth"]:
        ...
        # if logged in show Store and Retrieve pages
